local function ModifyDecompiledCode(code)
    ...
    local function RenameVariables(line)
        local varName = line:match("local%s+(%w+)%s*=%s*{}") or
                        line:match("local%s+(v_u_%d+)%s*=%s*{}") or
                        line:match("local%s+(%w+)%s*=%s*{") or
                        line:match("local%s+(v_u_%d+)%s*=%s*{")
        ...
        varName = line:match("local%s+(v_u_%d+)%s*=")
